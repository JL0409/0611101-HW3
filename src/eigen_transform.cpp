    ...
    while (ros::ok())
    {
       ...
       // std::cout << euler_angle_rad.transpose() << std::endl;
        ...
        Eigen::Matrix3d Rotation;
         Rotation << 1-2 * (Q_tmp.y() * Q_tmp.y() + Q_tmp.z() * Q_tmp.z()), 2 * (Q_tmp.x() * Q_tmp.y() - Q_tmp.w() * Q_tmp.z()), 2 * (Q_tmp.w() * Q_tmp.y() + Q_tmp.x() * Q_tmp.z()),
                                   2 * (Q_tmp.x() * Q_tmp.y() + Q_tmp.w() * Q_tmp.z()), 1-2 * (Q_tmp.x() * Q_tmp.x() + Q_tmp.z() * Q_tmp.z()), 2 * (Q_tmp.z() * Q_tmp.y() - Q_tmp.x() * Q_tmp.w()), 
                                  2 * (Q_tmp.x() * Q_tmp.z() - Q_tmp.w() * Q_tmp.y()), 2 * (Q_tmp.z() * Q_tmp.y() + Q_tmp.x() * Q_tmp.w()), 1-2 * (Q_tmp.x() * Q_tmp.x() + Q_tmp.y() * Q_tmp.y());

        world_point = Rotation * world_point;
